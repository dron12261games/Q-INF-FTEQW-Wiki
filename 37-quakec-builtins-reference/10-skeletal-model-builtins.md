...
* **skel** — `float`, skeletal object, индекс целевого скелета; в отличие от `skel_build`, передача `0` здесь не вызовет полного пересоздания и просто завершится неудачей.
* **numblends** — `int`, количество передаваемых структур `skelblend_t` для смешивания.
* **weights** — `skelblend_t *`, указатель на массив blend-дескрипторов; каждая запись может брать источник либо из `sourceskel`, либо из `sourcemodelindex`/модели самого `skel`, задаёт `firstbone`/`lastbone`, `prescale`, до четырёх наборов `animation[] + scale[] + animationtime[]`, а для HL-подобных моделей ещё `subblend[]` и `controllers[]`.
* **structsize** — `int`, размер одной структуры массива; обычно передается `sizeof(skelblend_t)`.

#### Описание и особенности работы

`skel_build_ptr` является pointer-вариантом `skel_build`, но требует уже существующий skeletal object и не создаёт его автоматически. Из явных проверок движок делает минимум: target skeleton должен быть валиден, а `structsize` — не меньше `sizeof(skelblend_t)`. Дальше каждый blend-элемент обрабатывается по отдельности: если `sourceskel` невалиден, исходная модель не найдена/не содержит костей или запись ничего не добавляет, такой элемент просто пропускается. Для non-relative skeleton действуют почти те же ограничения, что и у обычного `skel_build`: частичное обновление диапазона либо `prescale` на absolute skeleton приводят к раннему выходу. Важно и то, что builtin не делает глубокой валидации самого QC-указателя, поэтому массив должен уже лежать в корректной памяти VM и иметь правильную раскладку.
...
`skel_postmul_bones` существует рядом с `skel_postmul_bone` как внутренняя функция движка с той же общей идеей: пост-умножить матрицей подряд идущий диапазон костей. Но в штатных builtin-таблицах этот вызов закомментирован, а в публичных QC-дефах он не публикуется как обычный builtin. Поэтому точнее считать его не «сломанный, но официально доступный» функцией, а неэкспортированной возможностью. Для переносимого QuakeC-кода безопасный путь остаётся тем же, что и раньше: пройти диапазон циклом и вызвать `skel_postmul_bone` для каждой кости отдельно.
