...
#### Описание и логика работы
Строит цепочку из всех сущностей, у которых выбранное поле равно `match`. В отличие от `find`, здесь не сравниваются строки: движок сопоставляет само хранимое числовое значение, поэтому builtin подходит для `.float`, packed-флагов, entity reference и других полей, где важен сырой слот. Как и у `findchain`, поле связи будет перезаписано у каждого совпадения.
...
* **relative_ofs** — `vector`, точка приложения/смещение для импульса.

#### Описание и логика работы
Отправляет в backend rigid-body physics команду применить импульс к физическому объекту. Если physics engine не активен, builtin фактически ничего не делает; то же самое относится к сущностям, которые не обслуживаются `MOVETYPE_PHYSICS`. Используйте этот API для пинков, взрывов, рычагов и других эффектов, где важен именно физический импульс, а не простое присваивание `velocity`.

#### Практические сценарии использования
```qc
void(entity box, vector dir) KickPhysicsBox =
{
    physics_addforce(box, dir * 400, '0 0 0');
};
```
...
#### Описание и логика работы
Добавляет вращающий импульс rigid-body объекту. Как и `physics_addforce`, builtin зависит от подключённого physics backend и молча теряет смысл, если мир не запущен с поддержкой физики. Практически это способ закрутить бочку, лопасть или обломок так, чтобы вращение считалось самим physics engine.
...
#### Описание и логика работы
Включает или выключает обработку physics backend для `MOVETYPE_PHYSICS` entity. По документации это прямой способ снизить CPU cost для тел, которые временно должны «замереть» и не участвовать в симуляции. Сам объект при этом остаётся сущностью мира; builtin лишь переключает участие тела в стороне physics engine.
...
#### Описание и особенности работы
`empty` не является реальным работоспособным builtin'ом. В коде `engine\server\pr_cmds.c` эти слоты инициализируются заглушкой `PF_Fixme`, которая выводит сообщение о незанятом опкоде для номеров `#245..#249`. Соответственно, вызывать этот слот из QC не имеет смысла: это просто резервные слоты для расширения ядра и будущих встроенных функций.
...
#### Описание и особенности работы
Builtin контролирует флаг `readonly` в структуре эдикта и при активации блокирует любые попытки изменения значений переменных и полей со стороны QC. Архитектурно этот метод полезен, когда генерируются базовые шаблоны, но базовый функционал в `pr_bgcmd.c` механически присваивает значение переменной `nowreadonly` в логическое поле структуры `e->readonly`; возвращая при успешном изменении `0..1` статус самого флага. Попытка записи в защищенный эдикт вызовет ошибку в интерпретаторе, аналогичную модификации `world` или нулевых эдиктов, защищая важные системные ссылки.
...
### findentity
`entity(entity start, .__variant fld, __variant match) findentity = #98;`
...
#### Описание и особенности работы
В FTE-расширениях `findentity` часто выступает синонимом или расширенной версией `findfloat`, заменяя собой узкоспециализированные функции поиска. Builtin сканирует активные эдикты, начиная со следующего за `start`, проверяет соответствие указанного поля переданному значению и возвращает первый найденный эдикт; при отсутствии совпадений возвращается `world`. Метод эффективен для поиска по полям `.entity` и `.float` без накладных расходов на строковое приведение типов в интерпретаторе.
...
#### Описание и особенности работы
Альтернатива стандартной функции `findradius`, которая возвращает temp-массив сущностей вместо создания связанного списка через поле `.chain`. На стороне игрового движка FTEQW расчет ведется на основе bbox-пересечений объектов с областью вокруг `org`. Переменные `sv_gameplayfix_findradiusdistancetobox` и `dpcompat_findradiusarealinks` влияют на то, как именно этот builtin рассчитывает дистанцию, а non-solid сущности без флага `FL_FINDABLE_NONSOLID` автоматически игнорируются. Порядок возвращаемых элементов в массиве гарантирует быстрый и эффективный цикл обхода.
...
#### Описание и особенности работы
В современной логике FTEQW этот builtin не экспонируется: и в серверной, и в CSQC-частях он заблокирован из-за неполной реализации расширения `EXT_CSQC_1`. Попытка вызова приведет к системной ошибке, а официальная поддержка в текущих версиях отсутствует. На этапе проектирования это было экспериментальное API для непосредственного чтения server entity state, но в итоге разработчики QC используют стандартный сэмплинг.
...
`readsingleentitystate = #370; // закомментированный слот EXT_CSQC_1 для обратной совместимости`

* Параметры и возвращаемое значение не специфицированы: этот сетевой builtins заблокирован движком непосредственно при парсинге.

#### Описание и особенности работы
У `readsingleentitystate` абсолютно та же судьба, что и у `readserverentitystate`: в таблицах исходного кода движка этот слот `#370` присутствует, но при попытке вызова он выдаст критическое исключение. Это мертвый концепт-набросок, а не рабочий базовый builtin. Если вам нужна связь между сервером и клиентом, используйте стандартные механизмы Delta-сетевых сущностей или кастомные сетевые сообщения (sendentity).
...
* **ent** — `entity`, временная CSQC-сущность, которую нужно исключить из render-сцены до конца кадра.

#### Описание и особенности работы
Это не синоним стандартного удаления `remove`, а специфическая CSQC-команда клиентского рендеринга. Реализация в `pr_csqc.c` отвечает за удаление сущности из массивов `addentity`/`addentities` для rentity, используемых в текущем отрисовщике, что позволяет скрыть объект для split-screen, оптимизировать вывод сцены и динамически перегруппировывать отрисовку CSQC-моделей в процессе графического конвейера. В некоторых версиях движка при определенных условиях может выбрасываться предупреждение, но сам вызов `PF_Fixme` отсутствует; функция выполняет исключительно очистку на стороне CSQC.
...
#### Описание и особенности работы
Базовый builtin, который в коде компилятора и расширениях FTEQW часто объявляется как `runstandardplayerphysics`. Он запускает внутренний движковый обсчет player-physics, основываясь на значениях глобальных переменных `input_*` для текущего кадра, и обычно вызывается внутри функции `SV_RunClientCommand`. Если вам не нужен кастомный код движения, этот метод позволяет использовать нативную предсказуемую физику из коробки. Старое имя `runclientphys` в современных таблицах часто указывает на заблокированный опкод `#233` (`PF_NoCSQC`), поэтому для стабильной работы логики перемещения рекомендуется использовать именно этот обновленный builtin.
...
#### Описание и особенности работы
Аналогично слотам `empty`, метка `undefined` в исходном коде движка используется как заполнитель пустых позиций в глобальных массивах встроенных функций. В файлах `pr_cmds.c` и `pr_csqc.c` эти опкоды жестко перенаправлены на стандартный макрос `PF_Fixme`, поэтому попытка их явного вызова из QC-скрипта приведет к ошибке интерпретатора. Данные слоты зарезервированы исключительно для будущих расширений API движка и обратной совместимости.
