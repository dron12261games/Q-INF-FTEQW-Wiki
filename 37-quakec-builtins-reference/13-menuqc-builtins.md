...
#### Описание и логика работы
Для MenuQC `addentities` в текущем FTEQW по умолчанию не экспортируется: `checkbuiltin(addentities)` для `menu.dat` вернёт `FALSE`. Поэтому переносимый код меню не должен рассчитывать на этот builtin вовсе. Если нужен гарантированный 3D-фон меню, собирайте сцену вручную через `clearscene`, `setproperty`, `addentity` и `renderscene`.
...
void(vector screensize) m_draw =
{
    ...
    if (checkbuiltin(addentities))
        addentities(MASK_ENGINE); // в обычной сборке FTEQW сюда лучше не полагаться
...
#### Описание и логика работы
`setproperty` переопределяет параметры камеры и viewport для ближайшего `renderscene`: положение и углы камеры, поле зрения, область рендера, часть флагов отрисовки HUD/мира и связанные параметры. В документации MenuQC builtin обычно числится как доступный, но в конкретной сборке меню он может отсутствовать. Если он есть, то это ключевой инструмент для 3D-фона меню: сначала `clearscene`, затем `setproperty(VF_MIN, ...)`, `setproperty(VF_SIZE, ...)`, `setproperty(VF_ORIGIN, ...)`, `setproperty(VF_ANGLES, ...)`, затем `addentity` и `renderscene`.
...
#### Описание и логика работы
[`getresolution`](08-csqc-rendering-builtins.md#getresolution) помогает строить собственное меню видеонастроек без парсинга текстового вывода движка. Возвращаемый вектор содержит ширину в `x`, высоту в `y`, а в `z` — простой признак валидности режима: `1`, если размер найден, и `0`, если нет. Для полноэкранных режимов builtin обращается к списку режимов движка, а для `vidmode = -1` берёт параметры рабочего стола. Если индекс не существует, возвращаются нули.
...
* **width** — желаемая толщина линии; в текущей реализации параметр читается, но фактически игнорируется.
* **pos1**, **pos2** — начальная и конечная точки линии.
* **rgb** — цвет линии.
* **alpha** — прозрачность.
* **flag** *(optional)* — draw-флаг/режим смешивания.

#### Описание и логика работы
С [`drawline`](08-csqc-rendering-builtins.md#drawline) у MenuQC есть важный practical-нюанс: старые menu-headers нередко показывают только три аргумента, но текущая реализация FTEQW читает тот же расширенный набор, что и CSQC — цвет, alpha и необязательный drawflag. При этом сам `width` пока не влияет на толщину: линия рисуется фактически только по двум точкам и цвету. Поэтому в актуальной документации разумнее ориентироваться именно на расширенную сигнатуру и воспринимать `width` как зарезервированный параметр совместимости.
