...
Важная практическая деталь: variadic-хвост после `defaultshader` нужен не «для красоты», а для удобной сборки длинного shader script из нескольких строковых аргументов. Это позволяет писать читабельный QuakeC без ручного `strcat` для больших блоков текста.
