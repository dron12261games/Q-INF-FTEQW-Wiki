...
### CL_RotateMoves
`void(vector anglechange) CL_RotateMoves = #638;`

* **anglechange** — `vector`, дельта углов, на которую нужно довернуть углы обзора принудительно, но без изменения сформированных инпут-команд (input commands).

#### Описание и особенности работы
`CL_RotateMoves` — это специализированный CSQC builtin для коррекции внутренней истории входящих команд (input history) после резкого разворота игрока или смены ориентации. На стороне движка `PF_cl_RotateMoves` производит обход массива отправленных фреймов `cl.outframes`, у которых `cmd_sequence > cl.ackedmovesequence`, принудительно вычитая или прибавляя дельту `anglechange` к полю `usercmd.angles` в соответствии с поправкой угла. Это необходимо для плавной симуляции порталов, скриптовых перехватов камеры (scripted camera snaps) и телепортов: метод позволяет клиенту мгновенно скорректировать свои предсказанные углы (prediction), чтобы картинку не дергало в момент сетевого подтверждения. В отличие от других движков, этот метод корректно учитывает номер текущего локального игрока (seat), если игра запущена в режиме сплит-скрина (split-screen).

#### Пример использования
```qc
void(vector delta_ang) ApplyPortalTurn =
{
    // Меняем направление взгляда игрока при проходе через портал,
    // не ломая предсказание уже отправленных инпут-фреймов.
    CL_RotateMoves(delta_ang);
};
```
...
#### Описание и особенности работы
`clipboard_get` инициирует асинхронный запрос текстовых данных из системного буфера обмена операционной системы, однако сам текст напрямую функция не возвращает. В MenuQC полученная UTF-8 строка будет передана движком в стандартное событие ввода `Menu_InputEvent` в виде кода операции `CSIE_PASTE`, внутри которого разработчик должен посимвольно или построчно перехватить входящие данные через глобальные переменные ввода. Обратите внимание: в CSQC-контексте использование этой функции заблокировано разработчиками FTEQW. При попытке вызова движок выдаст лог `don't let csqc read the clipboard right now. its too risky.`, так как чтение буфера из игрового кода считается угрозой безопасности. На мобильных платформах метод привязан к нативным API вызова системного окна вставки.

#### Пример использования
```qc
string menu_paste_buffer;

float(float evtype, float scan, float unicode, float devid) Menu_InputEvent =
{
    if (evtype == CSIE_PASTE)
    {
        menu_paste_buffer = strcat(menu_paste_buffer, chr(unicode));
        return TRUE;
    }
...
void() Menu_RequestPaste =
{
    menu_paste_buffer = "";
    clipboard_get(CBT_CLIPBOARD);
};
```
...
* **cliptype** — `int`, целевой тип системного буфера обмена.
* **text** — `string`, UTF-8 строка, которую необходимо записать в буфер обмена.

#### Описание и особенности работы
`clipboard_set` отправляет указанный текст в системный буфер обмена ОС и стабильно поддерживается как в CSQC, так и в MenuQC. Метод вызывает внутреннюю сишную процедуру `Sys_SaveClipboard`, которая осуществляет передачу данных платформозависимому бэкенду; в файле `sys.h` исходников ядра указано, что на headless-серверах эта функция компилируется как безопасная пустая заглушка (stub). На десктопных операционных системах вызов мгновенно обновляет буфер Ctrl+C/Ctrl+V, а под Linux при использовании флага `CBT_SELECTION` данные дублируются в буфер выделения (selection buffer). Отлично подходит для реализации кнопок «Скопировать адрес сервера», «Скопировать URL дискорда» или экспорта игровых логов во внешний редактор.

#### Пример использования
```qc
void() Menu_CopyServerAddress =
{
    clipboard_set(CBT_CLIPBOARD, "connect example.org:27500");
};
```
...
* **pos** — `vector`, экранные 2D-координаты левого верхнего угла текстового блока.
* **size** — `vector`, ширина и высота ограничивающей рамки (bounding box) для текста.
* **alignflags** — `float`, битовые флаги выравнивания, переноса и форматирования; в стандартных расширениях FTEQW флаг автоматического переноса строк обозначается как `RTLB`, а классическое значение блочного выравнивания равно `3`.
...
#### Пример использования
```qc
void() Menu_DrawWrappedDescription =
{
    local float lines;

    lines = drawtextfield('64 96 0', '320 80 0', 3,
        "This block will wrap automatically and report how many lines were drawn.");
...
* **shadname** — `string`, имя медиа-шейдера или текстуры встроенного браузера.
* **propname** — `string`, имя свойства, которое запрашивается у декодирующего бэкенда (decoder backend).

#### Описание и особенности работы
`gecko_getproperty` запрашивает специфические для конкретного декодера свойства у мультимедийного бэкенда или встроенного веб-браузера. Архитектурная деталь движка гарантирует, что для встроенных CEF-браузеров (Chromium Embedded Framework) всегда доступны базовые ключи `"url"`, `"title"` и `"status"`. Внутренняя функция ядра `Media_Send_GetProperty` извлекает эти текстовые данные на лету, что позволяет синхронизировать воспроизведение синематиков или кастомных плагинов интерфейса по строковому ключу. Builtin одинаково доступен как в CSQC, так и в MenuQC, поскольку обе виртуальные машины оперируют графическим выводом веб-поверхностей и видеодекодеров. Если переданный шейдер не найден или бэкенд не поддерживает указанное свойство, функция безопасно возвращает пустую строку `""`.

#### Пример использования
```qc
void() Browser_DebugTitle =
{
    local string title;

    title = gecko_getproperty("browser/help", "title");
    if (title != "")
        print(sprintf("page title: %s\n", title));
};
```
...
* **effective** — `float`, флаг логического режима запроса: если установлен в `0`, функция возвращает режим курсора, заданный только текущей VM; если установлен в `TRUE`, возвращается итоговое фактическое состояние захвата мыши операционной системой.

#### Описание и особенности работы
`getcursormode` опрашивает состояние видимости и захвата курсора мыши в рамках оконной системы движка. При базовом вызове (`effective == 0`) builtin проверяет, запрашивала ли конкретно ваша VM отображение указателя через `setcursormode`. В режиме полного запроса (`effective == TRUE`) функция вызывает системную процедуру ядра `Key_MouseShouldBeFree()`, которая анализирует все активные слои инпута движка (консоль, игровое меню, оверлеи) и внешние факторы вроде touch-only устройств. Это критически важно при создании сложных перекрывающих друг друга интерфейсов, чтобы определить, действительно ли мышь освобождена для пользователя, или её перехватил другой приоритетный слой ввода. Работает в CSQC и MenuQC.

#### Пример использования
```qc
void() Menu_EnsureCursorVisible =
{
    setcursormode(TRUE, "", '0 0 0', 1);

    if (!getcursormode(TRUE))
        dprint("cursor is still captured by another input layer\n");
};
```
...
* **newpos** — `vector`, новые 2D-координаты указателя мыши в пределах экранного пространства (viewport).

#### Описание и особенности работы
`setmousepos` осуществляет программное позиционирование и перемещение курсора мыши в указанную точку экрана. Чаще всего применяется при переключении фокуса ввода на конкретное диалоговое окно, при обработке кастомных круговых меню (radial menus) или эмуляции абсолютных координат мыши. Координатная сетка автоматически масштабируется движком на основе текущего логического разрешения `vid.width/height`, преобразуясь во внутренние физические пиксели `vid.pixelwidth/height`. Стоит учитывать, что для ОС этот вызов является лишь хинтом: он инициирует аппаратный сдвиг (warping) курсора, результаты которого будут переданы в событие `*_InputEvent` с типом `IE_ABSMOUSE` на следующем кадре. Builtin эффективен для фиксации мыши в центре UI-элементов, но его не стоит вызывать каждый кадр во избежание сбоев системного ускорения мыши.

#### Пример использования
```qc
void() Menu_CenterCursorOnDialog =
{
    setcursormode(TRUE, "", '0 0 0', 1);
    setmousepos('320 240 0');
};
```
...
* **newcaption** — `string`, новый текст заголовка главного окна игры.

#### Описание и особенности работы
`setwindowcaption` динамически переписывает строку заголовка (title) окна приложения в операционной системе, изменяя текст на панели задач, в переключателе окон (Task Switcher) и декорациях оконного менеджера. Доступен в CSQC и MenuQC. На низком уровне вызов переопределяет глобальный буфер имени, и если заголовок меняется, ядро форсирует обновление через процедуру `CL_UpdateWindowTitle()`. Функция идеальна для вывода названия текущего игрового сервера в браузере серверов, отображения статуса загрузки файлов или кастомных информационных плашек внутри одиночных кампаний. Переданная строка должна быть в кодировке UTF-8, любые некорректные символы будут автоматически экранированы или отброшены системным API.

#### Пример использования
```qc
void() Menu_ShowBrowserCaption =
{
    setwindowcaption("FTEQW - Server Browser");
};
```
