...
### getinputstate
`float(float inputsequencenum, optional float seat) getinputstate = #345;`

* **inputsequencenum** — номер input-frame из клиентского журнала команд, который нужно загрузить в `input_*` globals.
* **seat** *(optional)* — номер локального игрока для split-screen; если аргумент опущен, используется текущий CSQC seat.

#### Описание и логика работы
`getinputstate` доступна только в CSQC и нужна прежде всего для prediction-кода. Она берёт сохранённый кадр пользовательского ввода, копирует его в глобалы `input_timelength`, `input_angles`, `input_movevalues`, `input_buttons`, `input_impulse` и возвращает ненулевое значение при успехе. Нормальный рабочий диапазон — `servercommandframe < sequence <= clientcommandframe`; при паузе, неверном `seat` или отсутствующем кадре builtin вернёт `0`.
...
* **usecursor** — `1`, если модулю нужен освобождённый видимый курсор; `0`, если мышь должна быть захвачена движком как обычный relative input.
* **cursorimage** — необязательное имя изображения курсора; пустая строка снимает ранее заданное кастомное изображение.
* **hotspot_and_scale** — необязательный вектор, где `x/y` задают hotspot, а `z` — масштаб курсора.

#### Описание и логика работы
`setcursormode` доступна в CSQC и MenuQC и просит движок либо отпустить мышь в абсолютный режим, либо снова захватить её для игры. Если указан `cursorimage`, FTEQW пытается использовать его как курсор; hotspot берётся из `x/y`, а масштаб — из `z` этого же вектора. Отдельного четвёртого scalar-параметра `scale` в текущей реализации нет.
...
void() Menu_EnablePointer =
{
	...
	setcursormode(1, "gfx/ui/cursor.tga", '0 0 1');
};
```
...
* **anglechange** — `vector`, дельта углов, на которую нужно довернуть уже сформированные команды.
* **seat** *(optional)* — номер локального игрока; если аргумент опущен, используется текущий CSQC seat.

#### Описание и особенности работы
`CL_RotateMoves` — специализированный CSQC builtin для коррекции истории ещё не подтверждённых usercmd. Движок проходит по `cl.outframes` с `cmd_sequence > cl.ackedmovesequence` и поворачивает сохранённые углы ввода, чтобы prediction не «ломался» после телепорта, портала или резкого скриптового поворота камеры.
...
#### Описание и особенности работы
`setmousepos` программно сдвигает курсор в указанную точку, но только если для текущего keydest уже активен absolute mouse mode. Координаты переводятся из логических `vid.width/height` во внутренние физические `vid.pixelwidth/height`.
