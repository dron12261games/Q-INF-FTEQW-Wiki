...
| [`skel_find_bone`](../37-quakec-builtins-reference/10-skeletal-model-builtins.md#skel_find_bone) | `float(float skel, string tagname)` | Находит номер кости по её имени в модели, использованной при создании скелетного объекта |
| [`skel_get_bonerel`](../37-quakec-builtins-reference/10-skeletal-model-builtins.md#skel_get_bonerel) | `vector(float skel, float bonenum)` | Возвращает смещение кости относительно родительской кости; ориентация дополнительно записывается в глобальные переменные `v_forward`/`v_right`/`v_up` |
