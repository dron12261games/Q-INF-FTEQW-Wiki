...
> [⬅ Предыдущая страница](experimental-d3d11-vulkan.md) | [Следующая страница ➡](lua-not-default.md)
...
- Подсистема physics backend'а (`USERBE`) включается только если сборка вообще содержит подходящий backend (`PLUGINS`, `USE_INTERNAL_BULLET` или `USE_INTERNAL_ODE`).
- Builtins физики собираются только когда этот backend-путь действительно включён.
